...
#TEMPORAL OVERLAP CALCULATED BETWEEN HUMAN AND EACH MAMMALS IN EACH HABITAT COVER
...
habitat<-read.csv("Site Habitat.csv", header=TRUE)
str(habitat)
View(habitat)

##### TEMPORAL OVERLAP FUNCTION ########
#POOLS HUMAN AND MAMMAL TAGS BY EACH HABITAT COVER-OPEN OR CLOSED
#COUNTS TOTAL NUMBER OF TAGS OF EACH SPECIES IN EACH HABITAT COVER
#CALCULATES TEMPORAL OVERLAP BETWEEN HUMANS AND EACH MAMMAL IN EACH HABITAT COVER, WITH LOWER AND UPPER CONFIDENCE INTERVALS
#PLOTS OVERLAP GRAPHS FOR HUMANS AND EACH MAMMAL IN EACH HABITAT COVER
...
HCtemporalOVERLAP<-function(exifNIGHT, mdFULL, habitat){
  
  #MERGE HABITATS TO DATASETS
  habitat$HabitatCover<-as.character(habitat$HabitatCover) #Convert Habitat Cover elements to characters from factors
  exifNIGHT<-merge(exifNIGHT, habitat, by.x="TagPlaceID", by.y="TagPlaceID") #Merge Habitat Cover elements to ExifPro data
  mdFULL<-merge(mdFULL, habitat, by.x="TagPlaceID", by.y="TagPlaceID") #Merge Habitat Cover elements to MegaDetector data
  ...
  exifNIGHT$DatesP <- as.POSIXct(exifNIGHT$Date, tz="GMT") #Convert dates to a POSIXct object for ExifPro data
  mdFULL$DatesP <- as.POSIXct(mdFULL$Date, tz="GMT") #Convert dates to a POSIXct object for MegaDetector data
  
  #DEFINE COORDINATES OF HAMPSTEAD HEATH AS A SPATIAL POINT, SO PACKAGE KNOWS WHICH SUNRISE AND SUNSET TIMES TO CORRECT FORimes to correct for
  ...
  colnames(RESULTS)<-c("HabitatCover", "Species", "Total Sightings", "Overlap", "LowerCI", "UpperCI") #Name columns of new results matrix
  HABITATcover<-sort(unique(exifNIGHT$HabitatCover)) #Create reference vector defining Closed and Open Habitat Covers to group by
  SpeciesNames<-c("Human", "Fox", "Hedgehog") #Vector of species for row names and later filtering
  RESULTS[,1]<-rep(HABITATcover, 3) #Column 1 ("Habitat Cover") filled systematically with Habitat Cover names
  ...
  for(s in 2:3){
    ...
    #WITH ONE MINUTE TIME INTERVAL APPLIED, SUBSET BY HABITAT COVER FOR HUMAN AND EACH MAMMAL
    #DETERMINE NUMBER OF TAGS, AND OVERLAP INDEX WITH CONFIDENCE INTERVALS AND PLOT GRAPHS
    for(j in 1:2){ #for each habitat cover (Open or Closed)
      h<-subset(HumanOM, HabitatCover==HABITATcover[j]) #filter human tags by those in specified habitat cover
      a<-subset(OM, HabitatCover==HABITATcover[j]) #filter specified mammal tags by those in specified habitat cover
      RESULTS[j,3]<-length(h$File) #record number of human tags in particular habitat cover in results matrix
      RESULTS[j+(s-1)*2,3]<-length(a$File)#record number of specified mammal tags in particular habitat cover in results matrix
      ...
      if(RESULTS[j,3]>=10 & RESULTS[j+(s-1)*2,3]>=10){ #only generate temporal overlap estimates if at least ten tags of human and foxes/hedgehogs
        ...
        setwd("C:/Users/beasl/Documents/Project/Project Data/FINAL CODES AND RESULTS/Final Codes/DATA AND CODES/3) Temporal Overlap/Habitat Cover") #change working directory to Results folder
        jpeg(paste(SpeciesNames[s], "vs Human Temporal Overlap-", HABITATcover[j], ".jpeg", sep=" "), width = 175, height = 150, units='mm', res = 300) #prime R to save temporal graph as a JPEG
        ...
        overlapPlot(h$Radians, a$Radians, xcenter = c("noon"), linewidth=c(2,2), linecol=c("darkblue", "red"), olapcol="lightgray", main=title, ylab="Proportion of Activity", xlab="Time of Day") #plot temporal overlap graph, with graph centred on noon, different coloured and textured line by species, and appropriate labels
        ...
        mysubtitle= HABITATcover[j]; mtext(side=3,line=0.3, mysubtitle) #add subtitle defining Habitat Cover used for the graph
    ...
    write.csv(RESULTS, "Temporal Overlap Results-Habitat Cover.csv") #save completed results matrix to directory
  }
}

HCtemporalOVERLAP(exifNIGHT,mdFULL, habitat)
